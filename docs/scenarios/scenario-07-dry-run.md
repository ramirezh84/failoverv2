...
| t (min) | Layer | Event |
|---|---|---|
| 0 | Test driver | Inject the scenario condition (see `tests/chaos/test_scenario_07_dry_run.py`). |
...
> **State machine runs every state; SNS subjects all prefixed `[DRY-RUN]`; no SSM/R53/Aurora changes; logs `dry_run_action_skipped` per side effect.**
...
Dry-run exercises every state without mutating the system.
