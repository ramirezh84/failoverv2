# Scenario 13 — Profile change mid-incident
...
| t (min) | Layer | Event |
|---|---|---|
| 0 | Test driver | Inject the scenario condition (see `tests/chaos/test_scenario_13_profile_change_mid_incident.py`). |
...
> **Decision Engine re-reads the profile every minute; within one polling interval, the rule no longer authorizes auto failover; SNS HIGH alerts continue.**
...
Profile changes propagate within the polling interval; no stale-profile actions.
