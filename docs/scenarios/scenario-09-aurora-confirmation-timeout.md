# Scenario 09 — Aurora confirmation timeout
...
| t (min) | Layer | Event |
|---|---|---|
| 0 | Test driver | Inject the scenario condition (see `tests/chaos/test_scenario_09_aurora_confirmation_timeout.py`). |
...
> **Executor reaches its `aurora_confirm_timeout_minutes`; FAIL state; CRITICAL SNS; no R53 rollback (per SPEC §6 invariant).**
...
System stays in known-stuck rather than rolling back DNS to a degraded primary.
