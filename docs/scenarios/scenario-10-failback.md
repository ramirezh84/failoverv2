# Scenario 10 — Failback after stable secondary
...
| t (min) | Layer | Event |
|---|---|---|
| 0 | Test driver | Inject the scenario condition (see `tests/chaos/test_scenario_10_failback.py`). |
...
> **Mirror workflow: drain secondary, quiesce, flip R53 back, promote primary indicator, Aurora gate, postcheck, STABLE_PRIMARY.**
...
Failback is symmetric to failover; both directions exercise the same Lambda set.
